...
async fn get_sourcers(
    ...
    if tokio::fs::create_dir_all(destination_path.join("assets/indexes"))
        .await
        .is_err()
    {
    ...
    if tokio::fs::create_dir_all(destination_path.join("assets/objects"))
        .await
        .is_err()
    {
        error!("{CANT_CREATE_DIR}");
...
pub async fn download_sourcers(
    ...
    let mut gen_downloader = Downloader::new(
        urls.into(),
        names,
