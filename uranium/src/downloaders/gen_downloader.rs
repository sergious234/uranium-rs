...
pub struct OldDownloader<T: Req + Clone + Send> {
...
impl<T: Req + Clone + std::marker::Send + std::marker::Sync + 'static> OldDownloader<T> {
    ...
    async fn get_responses(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        for url_chunk in self.urls.chunks(chunk_size) {
            ...
            let task = tokio::task::spawn(async {
                download_and_write(path_c, responses, names)
                    .await
...
impl<T: Req + Clone + std::marker::Send + std::marker::Sync + 'static> Downloader<T> {
    pub async fn start(&mut self) {
        loop {
            if self.get_responses().await.is_err() {
                break;
            }
        }
        //self.get_responses().await.unwrap();
    ...
    async fn make_requests(&mut self) -> Option<()> {
        ...
        let task = tokio::spawn(async {
            download_and_write(path, responses, names)
                .await
                .map_err(|e| eprintln!("{}", e))
                .unwrap();
        });
